...
;; pseudo main function
  logger := New Logging()     ;; The cache will be written to a text file
                              ;; allowing text formatting with markdown syntax tags: 
                              ;; bold = **text**, highlight = `text`, italic = _text_
  const := New Constants()
  app := New Application("bug.n-x.min", "0.0.3")
...
class Application {
  __New(name, version) {
    ...
    DetectHiddenWindows, Off
    logger.info("Window with id ``" . this.windowId . "`` identified as the one of " . this.name . "'s process.", "Application.__New")
  }
}
...
#Include, %A_ScriptDir%\configuration\custom.ahk
...
#Include, %A_ScriptDir%\user-interface.ahk
